...
def main() -> None:
    ...
    subprocess.run(
        [objcopy, "-I", "binary", "-O", "elf32-littlearm", "-B", "arm", str(args.binary), str(raw_obj), "--rename-section", ".data=.text,alloc,load,readonly,code"],
        check=True,
    )
    subprocess.run(
        [lld, "-m", "armelf", f"-Ttext=0x{args.load_address:08x}", "-e", f"0x{args.entry_address:08x}", "-o", str(args.output), str(raw_obj)],
        check=True,
    )
